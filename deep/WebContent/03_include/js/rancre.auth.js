...
Auth.checkNumber = function(mode) {
	...
	if (mode == 1) {
		if (check_ipt1.length < 3 || check_ipt2.length < 2
				|| check_ipt3.length < 5) {
...
Auth.businessCheck = function() {
	...
	if (check_ipt1.length == 3 && check_ipt2.length == 2
		...
		var businessNo = $("#inputCompany-Number1").val()
				+ $("#inputCompany-Number2").val()
				+ $("#inputCompany-Number3").val();
...
Auth.join = function() {
	...
	businessNumber = protectXSS($("#inputCompany-Number1").val()
			+ $("#inputCompany-Number2").val()
			+ $("#inputCompany-Number3").val());
				...
				success : function(response) {
					...
					if (response.outputResult == "1") {
						...
						location.href = "/";
							...
							|| response.outputResult == "-4") {
						$("div.msgRow.malfunMsg").text(
								"이미 가입된 이메일 또는 사용중인 닉네임입니다.");
					} else {
...
Auth.userInquiry = function() {
	...
	$
			.ajax({
				...
				async : false,
				success : function(response) {
					if (response.outputResult == "1") { // valid email
						// information
						alert("인증 메일이 발송되었습니다. 인증메일을 확인해주세요.");
						location.href = "/";
